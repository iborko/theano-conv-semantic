...
Sažimanje kod dubokih neuronskih mreža je vrsta poduzorkovanja gdje se ulazna slika (ili mapa značajki) dijeli na više nepreklapajućih pravokutnika i za svaki pravokutnik vrati se jedna vrijednost. Postoji više vrsta sažimanja, a najčešće se koriste sažimanje maksimalnog odziva \engl{max pooling} i sažimanje prosjekom \engl{average pooling}.
...
\end{lstlisting}
gdje je \texttt{network.conf} konfiguracijska datoteka u JSON formatu. Primjer datoteke je prikazan u dodatku \ref{chap:konfiguracijska datoteka}
...
\begin{description}
  ...
  \item[normalize\_block] normalizira sliku tako da svaki $15 \cdot 15$ blok piksela svakog kanala ima srednju vrijednost intenziteta 0 i standardnu deviaciju 1
\end{description}
...
\begin{tabular}{l r r r}
  ...
  Konv. mreža s 3 razine & 75.7 & 59.3 & 0.2 \\
    \multicolumn{2}{c}{negativna log izglednost} & & \\ \hline
  Konv. mreža s 3 razine + \textit{superpixels} & 76.1 & 59.7 & 0.3 \\
    \multicolumn{2}{c}{negativna log izglednost} & & \\ \hline
  Konv. mreža s 3 razine & 73.8 & 60.5 & 0.2 \\
...
\begin{tabular}{l r r r}
  ...
  Konv. mreža s 3 razine (RGB) & 73.6 & 42.4 & 0.06 \\
    \multicolumn{2}{c}{negativna log izglednost} & & \\ \hline
  Konv. mreža s 3 razine (RGB) & 69.8 & 55.3 & 0 \\
    \multicolumn{2}{c}{Bayesova log izglednost} & & \\ \hline
  Konv. mreža s 3 razine (RGBD) & 72.0 & 46.7 & 0 \\
    \multicolumn{2}{c}{negativna log izglednost} & & \\ \hline
  Konv. mreža s 3 razine (RGBD) & - & - & 0 \\
    \multicolumn{2}{c}{Bayesova log izglednost} & & \\ \hline \hline
...
Područje dubokih neuronskih mreža nedavno se počelo ubrzano razvijati, a u mnogim područjima računalnog vida već postiže odlične rezultate. U ovom radu su opisane neuronske mreže, s naglaskom na konvolucijske neuronske mreže. Opisane su i metode učenja neuronskih mreža te mnogi detalji oko njihovog korištenja.

U sklopu ovog rada implementiran je cjelokupni sustav za semantičku segmentaciju korištenjem konvolucijskih neuronskih mreža. Sustav je realiziran korištenjem radne okoline Theano koja omogućuju pokretanje koda na grafičkim karticama s podrškom za CUDA jezik. Izgrađeni sustav se sastoji od slojeva neuronskih mreža poput konvolucijskih slojeva, slojeva sažimanja, višeslojnog perceptron, multinomijalne regresije i slično, koji se mogu kombinirati u razne arhitekture.

Arhitektura korištena u ovoj implementaciji napravljena je po uzoru na arhitekturu koju su predložili Farabet et al.\cite{farabet_pami}. Osim same mreže napisane su razne gradijentne metode poput stohastičkog gradijentnog spusta i Rmsprop-a, mnoge aktivacijske funkcije, skripte za predprocesiranje slika i postprocesiranje rezultata.
